...
以上で見たようにある関数を積分する際に適当な分布を持ってくることで効率よく行えることがわかった。
しかしながら、問題は、その分布からのサンプリング方法である。
累積分布を求めその逆関数としてサンプリングする**逆関数法**や、正規分布のような特定の分布に対しては
**Box-Muller法**のようなサンプリング法が知られているが、一般的な分布についてサンプリングする方法は
知られていない。この節ではそのようなサンプリングが難しい分布を、サンプリングが簡単な分布から近似的に
実行するマルコフ連鎖モンテカルロ法について解説する。

### マルコフ連鎖

**定義 0**: 与えられた状態空間 $S$ に値を取る確率過程 $\{X_n; n = 1, 2, \cdots\}$ が任意の時刻
$0 \leq n_1 < \cdots < n_k < n < m$ と状態 $i_1, \cdots, i_k, i, j \in S$ に対して

$$
P(X_m = j | X_{n_1} = i_1, \dots, X_{n_k} = i_k, X_n = i) = P(X_m = j | X_n = j)
$$

を満たす時、確率変数列 $\{X_n\}$ を $S$ 上のマルコフ連鎖という。
この式が意味するところは、時刻 $m$ で状態 $j$ になる確率は、直前の時刻 $n$ での状態 $i$ にだけ依存する、
ということだ。ベイズの定理により $P(X_{n_1} = i_1, \dots, X_{n_k} = i_k)$ はマルコフ連鎖の定義によって
このような条件付き分布の積で記述される。特に

$$
P(X_{n+1} = j | X_n = i) \equiv \mathcal{P}_{ij}^{(n)}
$$

を状態 $i$から $j$ への**遷移確率**といい、その行列表示 $\mathcal{P}_{ij}^{(n)}$ を**遷移確率行列** という。さらにこれが時間 $n$ によらず一定であるとき、
マルコフ連鎖は**時間的に一様**または**定常的**であるという。定常な遷移確率行列を $n$ を落として $\mathcal{P}_{ij}$ と表す。
定義から明らかに

$$
\mathcal{P}_{ij} \geq 0,
\quad
\sum_{j \in S} \mathcal{P}_{ij} = 1,
$$
...
で定義されるベクトルを時刻　$n$ における**分布**という。特に $X_0$ の定める分布　$\pi_i^{(0)}$ を
初期分布という。
...
$\lambda_\text{PF}$, $\boldsymbol v_\text{PF}$ と書く。
